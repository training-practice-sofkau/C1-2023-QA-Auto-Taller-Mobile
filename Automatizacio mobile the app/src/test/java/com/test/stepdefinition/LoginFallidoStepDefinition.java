...
import com.test.question.MensajeLoginFallido;
...
public class LoginFallidoStepDefinition extends SetUp {
    ...
    public void seLlenanLosCamposConYConCredencialesInvalidas(String user, String password) {
        try {
            actor.attemptsTo(
                    navigateToLogin(),
                    iniciarSesion()
    ...
    public void obtenemosUnMensajeDeLogueoFallido(String mensaje) {
        try {
            actor.should(
                    seeThat("Mensaje de inicio de sesion fallido",
                            MensajeLoginFallido.isEqualTo(),
                            equalTo(mensaje))
            );
            LOGGER.info("No se inicio Sesion, Asercion exitosa");
        } catch (Exception e){
